...
def register_page():
    ...
    if form.validate_on_submit():
        usert_to_create = User.create(user_id=str(uuid4()), name=form.name.data, username=form.username.data,
                                      email=form.email.data, password=bcrypt.generate_password_hash(form.password1.data).decode('utf-8'))
